...
export class LeafletMap extends React.Component {
  ...
  fetchJson() {
    ...
    return Promise.all(layers.filter(this.filterNonLayers).map(layer => {
      return new Promise(resolve => {
        get(layer.props.dataSource).then(json => {
          ...
          if (layer.props.cluster === 'true') {
            ...
            newLayerGroup = new L.MarkerClusterGroup(config.markerClusterConfig);
            newLayerGroup.cluster = true;
            newLayerGroup.clusterPolygons = this.createPolygons(features);

            newLayerGroup.addLayers(centerPoints);
          } else {
            newLayerGroup = new L.layerGroup();
